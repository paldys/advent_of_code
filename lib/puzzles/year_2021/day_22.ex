defmodule AdventOfCode.Puzzles.Year2021.Day22 do
  @moduledoc """
  ...
  of cubes, one cube per integer 3-dimensional coordinate (x,y,z). Each cube can
  ...
  other steps are fully within it. After executing these steps in
  the initialization procedure region, 590784 cubes are on.
  ...
  def parse(input) do
    ...
    |> Enum.map(fn line ->
      [action | coordinates] =
        Regex.run(
          ~r/^(on|off) x=(-?[[:digit:]]+)..(-?[[:digit:]]+),y=(-?[[:digit:]]+)..(-?[[:digit:]]+),z=(-?[[:digit:]]+)..(-?[[:digit:]]+)$/,
          line,
