defmodule AdventOfCode.Puzzles.Year2021.Day02 do
  @moduledoc """
  --- Day 2: Dive! ---
  ...
  def solve2(commands) do
    {position, depth, _} =
      List.foldl(commands, {0, 0, 0}, fn {command, units}, {position, depth, aim} ->
        case command do
          :forward -> {position + units, depth + aim * units, aim}
          :down -> {position, depth, aim + units}
          :up -> {position, depth, aim - units}
        end
      end)
