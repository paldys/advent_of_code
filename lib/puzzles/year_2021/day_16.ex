defmodule AdventOfCode.Puzzles.Year2021.Day16 do
  @moduledoc """
  ...
  a single binary number. To do this, the binary number is padded with leading
   zeroes until its length is a multiple of four bits, and then it is broken
   into groups of four bits. Each group is prefixed by a 1 bit except the last
   group, which is prefixed by a 0 bit. These groups of five bits immediately
  ...
  binary data represents its sub-packets, an operator packet can use one
  of two modes indicated by the bit immediately after the packet header;
  ...
  VVVTTTILLLLLLLLLLLAAAAAAAAAAABBBBBBBBBBBCCCCCCCCCCC
      ...
      - The 11 bits labeled L (00000000011) contain the number
      of sub-packets, 3.
      - The 11 bits labeled A contain the first sub-packet, a literal value
  ...
  Literal values (type ID 4) represent a single number as described above. The remaining type IDs are more interesting:
      ...
      of multiplying together the values of their sub-packets. If they only have
      ...
      the value of the first sub-packet is greater than the value of
      the second sub-packet; otherwise, their value is 0. These packets always
  ...
  def solve1(transmission) do
    transmission_bits = Base.decode16!(transmission)
    {decoded_packet, _} = decode_packet(transmission_bits)
    version_sum(decoded_packet)
  end

  def solve2(transmission) do
    transmission_bits = Base.decode16!(transmission)
    {decoded_packet, _} = decode_packet(transmission_bits)

    evaluate(decoded_packet)
  end

  defp evaluate(packet) do
    case packet do
      {:sum, _, subs} -> Enum.map(subs, &evaluate/1) |> Enum.sum()
      {:product, _, subs} -> Enum.map(subs, &evaluate/1) |> Enum.product()
      {:minimum, _, subs} -> Enum.map(subs, &evaluate/1) |> Enum.min()
      {:maximum, _, subs} -> Enum.map(subs, &evaluate/1) |> Enum.max()
      {:literal, _, value} -> value
      {:greater_than, _, [sub1, sub2]} -> if evaluate(sub1) < evaluate(sub2), do: 1, else: 0
      {:less_than, _, [sub1, sub2]} -> if evaluate(sub1) > evaluate(sub2), do: 1, else: 0
      {:equal_to, _, [sub1, sub2]} -> if evaluate(sub1) == evaluate(sub2), do: 1, else: 0
    end
  end

  defp version_sum(packet, versions \\ 0)

  defp version_sum({:literal, version, _}, versions) do
    versions + version
  end

  defp version_sum({_, version, subs}, versions) do
    sub_versions =
      Enum.map(subs, fn sub -> version_sum(sub, 0) end)
      |> Enum.sum()

    versions + version + sub_versions
  end

  def decode_packet(<<version::size(3), type::size(3), rest::bitstring>>) do
    case type do
      4 ->
      ...
      _ ->
        {subpackets, rest} = decode_operator(rest)
        operator = Map.get(@operators, type)
        {{operator, version, subpackets}, rest}
    end
  end

  defp decode_literal(literal, ret \\ 0)

  defp decode_literal(<<0::size(1), value::size(4), rest::bitstring>>, ret) do
    {(ret <<< 4) + value, rest}
  end

  defp decode_literal(<<1::size(1), value::size(4), rest::bitstring>>, ret) do
    decode_literal(rest, (ret <<< 4) + value)
  end

  defp decode_operator(<<0::size(1), value::size(15), rest::bitstring>>) do
    decode_operator_until(rest, bit_size(rest) - value)
  end

  defp decode_operator(<<1::size(1), value::size(11), rest::bitstring>>) do
    decode_operator_steps(rest, value)
  end

  defp decode_operator_until(transmission, stop, ret \\ [])

  defp decode_operator_until(transmission, stop, ret) when bit_size(transmission) <= stop do
    {ret, transmission}
  end

  defp decode_operator_until(transmission, stop, ret) do
    {value, rest} = decode_packet(transmission)
    decode_operator_until(rest, stop, [value | ret])
  end

  defp decode_operator_steps(transmission, steps, ret \\ [])

  defp decode_operator_steps(transmission, 0, ret) do
    {ret, transmission}
  end

  defp decode_operator_steps(transmission, steps, ret) do
    {value, rest} = decode_packet(transmission)
    decode_operator_steps(rest, steps - 1, [value | ret])
  end
end
