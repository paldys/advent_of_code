defmodule AdventOfCode.Puzzles.Day031 do
  ...
  def solve(codes) do
    gamma_epsilon_sum = List.duplicate(0, length(hd(codes))) # ge
    gamma_epsilon_sum = List.foldl(codes, gamma_epsilon_sum, fn code, ge ->
  ...
  defp gamma_epsilon([head | tail], {gamma, epsilon}) do
    {new_gamma, new_epsilon} = if head > 0 do
      {2 * gamma + 1, 2 * epsilon}
    else
      {2 * gamma, 2 * epsilon + 1}
    end
