defmodule AdventOfCode.Puzzles.Year2021.Day20 do
  @moduledoc """
  --- Day 20: Trench Map ---
  ...
  a single line, but it has been wrapped to multiple lines in this example
  ...
  the value of the pixel at (5,10) in the output image, nine pixels from
  the input image need to be considered: (4,9), (4,10), (4,11), (5,9), (5,10),
  (5,11), (6,9), (6,10), and (6,11). These nine input pixels are combined into
  a single binary number that is used as an index in the image enhancement
  algorithm string.
  ...
  pixels (.) into 0 and light pixels (#) into 1, the binary number 000100010 can
  ...
  image; the rest of the input image consists of dark pixels (.).
  ...
  def parse(input) do
    [raw_algorithm | raw_image] = String.split(input, "\n", trim: true)

    algorithm =
      String.to_charlist(raw_algorithm)
      |> Enum.map(fn c -> c == ?# end)
      |> Arrays.new()

    image =
      Enum.map(raw_image, fn line ->
  ...
  def solve1({algorithm, image}, steps \\ 2) do
    image = extend_image(image, steps)
    image = enhance_image(image, algorithm, false, steps)

    Enum.map(ArrayUtils.to_list(image), fn row ->
      Enum.count(ArrayUtils.to_list(row), & &1)
    end)
    |> Enum.sum()
  end
  ...
  defp extend_image(image, steps) do
    width = Arrays.size(image[0]) + steps * 2

    image_frame =
      List.duplicate(false, width)
      |> List.duplicate(steps)
      |> Enum.map(fn empty_line -> Arrays.new(empty_line) end)

    line_frame = List.duplicate(false, steps)

    image_core =
      Enum.map(ArrayUtils.to_list(image), fn row ->
        Arrays.new(line_frame ++ ArrayUtils.to_list(row) ++ line_frame)
      end)

    Arrays.new(image_frame ++ image_core ++ image_frame)
  end

  defp enhance_image(image, _, _, 0), do: image

  defp enhance_image(image, algorithm, infinity_pixel, steps) do
    height = Arrays.size(image)
    width = Arrays.size(image[0])

    updated_image =
      Enum.reduce(0..(height - 1), image, fn x, updated_image ->
        Enum.reduce(0..(width - 1), updated_image, fn y, updated_image ->
          put_in(updated_image[x][y], algorithm[enhancement_index(image, infinity_pixel, x, y)])
        end)
      end)

    infinity_pixel = if infinity_pixel, do: algorithm[511], else: algorithm[0]
    enhance_image(updated_image, algorithm, infinity_pixel, steps - 1)
  end

  defp enhancement_index(image, infinity_pixel, x, y) do
    for i <- -1..1, j <- -1..1 do
      is_light_pixel =
        if x + i < 0 or y + j < 0 or image[x + i][y + j] == nil do
          infinity_pixel
        else
          image[x + i][y + j]
        end

      if is_light_pixel, do: 1, else: 0
    end
    |> Enum.reduce(0, fn pixel, index -> 2 * index + pixel end)
  end
end
