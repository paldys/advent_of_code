defmodule AdventOfCode.Puzzles.Year2021.Day10 do
  @moduledoc """
  ...
  defp parse_chunk([], stack) do
    {:incomplete, stack}
  end

  def solve2(chunks) do
    incomplete_scores =
      Enum.map(chunks, fn chunk -> parse_chunk(chunk) end)
      |> Enum.filter(fn ret ->
        case ret do
      ...
      |> Enum.map(fn {_, closing_chars} -> evaluate_score(closing_chars) end)
      |> Enum.sort()

    Enum.at(incomplete_scores, div(length(incomplete_scores) - 1, 2))
  end

  defp evaluate_score(closing_chars) do
    Enum.reduce(closing_chars, 0, fn closing_char, acc ->
      acc * 5 + Map.get(@closing_char_points, closing_char)
    end)
  end
end
