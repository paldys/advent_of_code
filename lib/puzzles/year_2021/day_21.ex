defmodule AdventOfCode.Puzzles.Year2021.Day21 do
  @moduledoc """
  --- Day 21: Dirac Dice ---
  ...
  a circular track containing ten spaces marked 1 through 10 clockwise. Each
  ...
  order of increasing value, wrapping back around to 1 after 10). So, if
  a player is on space 7 and they roll 2, 2, and 1, they would move forward 5
  ...
  this point, the losing player had 745 points and the die had been rolled
  ...
  def parse(input) do
    ...
    |> Enum.map(fn line ->
      Regex.run(
        ~r/^Player [[:digit:]]+ starting position: ([[:digit:]]+)$/,
        line,
  ...
  def solve1({p1_position, p2_position}) do
    {loser_points, rolls} = play_turn(p1_position, p2_position)
    loser_points * rolls
  end
  ...
  defp play_turn(_, _, p1_points, p2_points, _, _, rolls)
       when p1_points >= 1000 or p2_points >= 1000 do
    {min(p1_points, p2_points), rolls}
  end

  defp play_turn(p1_position, p2_position, p1_points, p2_points, :p1, die, rolls) do
    p1_position = no_null_rem(p1_position + die, 10)
    p1_points = p1_points + p1_position
    die = if die == 0, do: 9, else: die - 1
    play_turn(p1_position, p2_position, p1_points, p2_points, :p2, die, rolls + 3)
  end

  defp play_turn(p1_position, p2_position, p1_points, p2_points, :p2, die, rolls) do
    p2_position = no_null_rem(p2_position + die, 10)
    p2_points = p2_points + p2_position
    die = if die == 0, do: 9, else: die - 1
    play_turn(p1_position, p2_position, p1_points, p2_points, :p1, die, rolls + 3)
  end
