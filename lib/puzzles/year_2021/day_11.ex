defmodule AdventOfCode.Puzzles.Year2021.Day11 do
  @moduledoc """
  ...
  the top-left octopus has an energy level of 5, the bottom-right one has
  ...
  def solve1(_, 0, flashes, _) do
    flashes
  end

  def solve1(energy_levels, steps, flashes, nil) do
    solve1(energy_levels, steps, flashes, init_index_cache(energy_levels))
  end

  def solve1(energy_levels, steps, flashes, index_cache) do
    {updated_energy_levels, new_flashes} = simulate_step(energy_levels, index_cache)
    solve1(updated_energy_levels, steps - 1, flashes + new_flashes, index_cache)
  end

  def solve2(energy_levels, steps \\ 0, index_cache \\ nil, size \\ nil)

  def solve2(energy_levels, steps, nil, nil) do
    rows = Arrays.size(energy_levels)
    cols = Arrays.size(energy_levels[0])
    solve2(energy_levels, steps + 1, init_index_cache(energy_levels), rows * cols)
  end

  def solve2(energy_levels, steps, index_cache, size) do
    case simulate_step(energy_levels, index_cache) do
      {_, ^size} -> steps
      {updated_energy_levels, _} -> solve2(updated_energy_levels, steps + 1, index_cache, size)
    end
  end

  defp init_index_cache(energy_levels) do
    rows = Arrays.size(energy_levels)
    cols = Arrays.size(energy_levels[0])
  ...
  defp simulate_step(energy_levels, energy_to_increase, flashes \\ [])
  ...
  defp simulate_step(energy_levels, [], flashes) do
    Enum.reduce(flashes, {energy_levels, 0}, fn {x, y}, {energy_levels, flashes} ->
      {put_in(energy_levels[x][y], 0), flashes + 1}
    end)
  end

  defp all_adjacent({x, y}, energy_levels) do
    rows = Arrays.size(energy_levels)
    cols = Arrays.size(energy_levels[0])
    ...
    for(
      ...
      y_delta <- -1..1,
      x_delta != 0 || y_delta != 0,
      x + x_delta >= 0 && x + x_delta < rows,
      y + y_delta >= 0 && y + y_delta < cols,
      do: {x + x_delta, y + y_delta}
    )
  end
end
