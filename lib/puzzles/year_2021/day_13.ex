defmodule AdventOfCode.Puzzles.Year2021.Day13 do
  @moduledoc """
  ...
  manage to find the manual; as you go to open it, page 1 falls out. It's
  a large sheet of transparent paper! The transparent paper is marked with
  random dots and includes instructions on how to fold it up
  ...
  def parse(input) do
    ...
    dots =
      Enum.map(raw_dots, fn raw_dot ->
        String.split(raw_dot, ",")
        |> Enum.map(&String.to_integer/1)
      end)
      |> Enum.map(&List.to_tuple/1)

    instructions =
      Enum.map(raw_instructions, &String.split(&1, "="))
      |> Enum.map(fn [raw_direction, amount] ->
        direction = if raw_direction == "fold along x", do: :vertical, else: :horizontal
  ...
  def solve1({dots, instructions}) do
    dots = MapSet.new(dots)

    Enum.take(instructions, 1)
  ...
  defp fold({:horizontal, amount}, dots) do
    Enum.reduce(dots, MapSet.new(), fn {x, y}, dots ->
      if y > amount do
        MapSet.put(dots, {x, 2 * amount - y})
      else
        MapSet.put(dots, {x, y})
      end
    end)
  ...
  defp fold({:vertical, amount}, dots) do
    Enum.reduce(dots, MapSet.new(), fn {x, y}, dots ->
      if x > amount do
        MapSet.put(dots, {2 * amount - x, y})
      else
        MapSet.put(dots, {x, y})
      end
    end)
